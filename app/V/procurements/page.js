...
export default function ProcurementsScreen() {
    ...
    const [msmeValue, setMsmeValue] = useState(secureLocalStorage.getItem("msme"));
    const [womenOwnedValue, setWomenOwnedValue] = useState(secureLocalStorage.getItem("womenOwned"));
    const [scstValue, setScstValue] = useState(secureLocalStorage.getItem("scst"));

    const [vendorOrganizationValue, setVendorOrganizationValue] = useState(secureLocalStorage.getItem("vendorOrganization"));
    const [vendorIDValue, setVendorIDValue] = useState(secureLocalStorage.getItem("vendorID"));
                ...
                <header className="absolute inset-x-0 top-0 z-50">
                    <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
                        ...
                        <div className="lg:flex lg:flex-1 lg:justify-end space-x-1">
                            <Link href={"/V"} className="bg-[#000000] text-[#ffffff] rounded-xl p-2 items-center align-middle flex flex-row hover:bg-[#3b3b3b] ">
                    ...
                    <div className="mx-auto max-w-2xl pt-16 lg:pt-24 ">
                        <div className="text-center">
                            <h1 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-4xl">
                                {"Procurements" + ` | ${vendorOrganizationValue}`}
                            </h1>
                            <p className="text-md bg-blue-50 rounded-xl p-2 m-4 w-fit text-[#131c48] ml-auto mr-auto">
                                {"Vendor ID -> " + vendorIDValue}
                            </p>
                        </div>
                    </div>
                                    ...
                                    return (
                                        <tr key={index} className="text-center">
                                            ...
                                            <td className={"border border-gray-200 px-1 py-1 flex-col justify-center space-x-2"}>
                                                <span>{procurement["goodsType"] ?? "-"}</span></td>
                                            <td className={"border border-gray-200 px-1 py-1 flex-col justify-center space-x-2"}><span className="bg-green-50 rounded-md p-1 w-fit text-[#143a0e]">{`${procurement["goodsQuantity"] ?? "-"}`}</span></td>
                                            ...
                                            <td className={"border border-gray-200 px-1 py-1" + (index === procurementDataFiltered.length - 1 ? " border-separate rounded-br-2xl" : "")}>
                                                <div className="flex flex-wrap">
                                                    ...
                                                    {msmeValue === '0' && womenOwnedValue === '0' && scstValue === '0' ? (
